/**
File: AgentSimulationExtension.java
Author: Hesed Guwn
Date: 09/27/2022
Project03
Course: CS231 B
...
public class AgentSimulationExtension {
    public static void main(String[] args) throws InterruptedException
    {
        ...
        else
        {
            ...
            LandscapeExtension scape = new LandscapeExtension(width, height, socialAgentNeighborTolerance, antiAgentNeighborTolerance);
            Random gen = new Random();

            // Creates 100 SocialAgents and 100 AntiSocialAgents (200 Agents in total)
            for (int i = 0; i < agents; i++) 
            {
                scape.addAgent(new SocialAgentExtension(gen.nextDouble() * scape.getWidth(),
                        gen.nextDouble() * scape.getHeight(),
                        socialRadius));
                scape.addAgent(new AntiSocialAgentExtension(gen.nextDouble() * scape.getWidth(),
                        gen.nextDouble() * scape.getHeight(),
                        antiSocialRadius));
            }
